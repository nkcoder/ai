...
Launch all reviewers in a single message using the Agent tool. Use the `interrogate reviewers` list from `~/.claude/rules/dstack-models.md` when present, one reviewer per entry, extending or shrinking the Reviewer A/B/C/D labels below to the configured entry count. Otherwise use the table defaults. Claude Code can only spawn Claude-family models, so the diversity signal here comes from spreading reviewers across Claude's distinct model tiers rather than across vendors.
