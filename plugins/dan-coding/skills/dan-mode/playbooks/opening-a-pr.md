...
**PRs.** Run `/simplify` over the diff before commit. Run `/no-comments` before review. Run `/security-review` before the PR opens when the diff touches a trust boundary (Non-negotiables). Write every PR title, PR description, and commit body with `/technical-writing`, then apply `/unslop`. Apply every technical-writing layer except Diátaxis. Use one word for each action, keep articles, and avoid `-ing` when a plain verb works.

**Titles.** Use Conventional Commits in the form `type(scope): subject`. Use `feat`, `fix`, `docs`, `refactor`, `test`, `chore`, `perf`, or `revert` as the type. Use the changed area, such as `dstack` or `dan-mode`, as the scope. Keep the subject short and imperative. Name a real symbol when one carries the change. For example, `fix(dstack): retarget opening-a-pr babysit trigger`. Do not add a trailing period.
...
- `## Verification`. Name each real run path and its outcome. Name the security gate's verdict when it applied, or say the diff touches no trust boundary. For a performance change, report one primary number with its unit in `before → after` form. Link the arena or swarm directory for the remaining evidence. Do not include sample-size methodology, swarm recitals, or metric tables.
...
A subagent that opens a PR runs `interrogate`, `/simplify`, `/no-comments`, and `/security-review` when the diff touches a trust boundary. It returns the URL and does not babysit. Return to the parent.
