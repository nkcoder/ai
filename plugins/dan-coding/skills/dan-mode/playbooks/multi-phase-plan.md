...
6. Run `node plugins/dan-coding/skills/dan-mode/scripts/check-plan.mjs <plan.md>` and fix every line it prints (the **encode-lessons-in-structure** principle skill).
...
**Control skill.** Pick it by surface. The project's own `verify-<app>` skill wins where it exists, since its feature map names the entry points a lane has to cover. Otherwise browser, Electron, CLI, TUI, and other project surfaces use the **run** skill. Native mobile uses whatever simulator-driving skill the repo has. A PR that touches two surfaces gets lanes on both. A surface with no control skill gets one built first, as its own PR at the front of the program, via the **create-verification-skill** skill. Ten lanes driving an ungenerated harness is ten lanes of improvisation. Only a surface that skill cannot reach stays a risk in Appendix C, and its live block still names how each lane drives it.
...
The program runs `skills/dan-mode/playbooks/<execution playbook>.md`. <Who merges, and which PR ids are the operator's items that stop at merge-ready.>
...
- [ ] Read these from trunk at program start. Re-read them at every tick.
  - [ ] `git show origin/main:skills/dan-mode/playbooks/<execution playbook>.md`
  - [ ] `git show origin/main:skills/swarm/SKILL.md`
  - [ ] `git show origin/main:<control skill path>`
  - [ ] `git show origin/main:skills/dan-mode/playbooks/opening-a-pr.md`
