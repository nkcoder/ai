...
Before spawning investigators, list the available MCPs. Use the available-tools map when present. Otherwise run `claude mcp list` (or check `.mcp.json` in the project and `~/.claude.json`) for configured MCP servers.
...
- `subagent_type`: `general-purpose` (full tool access by default, including MCP — required since investigators are MCP-backed)
- `model`: your configured why-investigators model (default `claude-sonnet-5-thinking-high`)
- Investigators shouldn't write anything, even though the agent type permits it.
...
- `subagent_type`: `general-purpose` (full tool access by default, including MCP)
- `model`: your configured why-synthesizer model (default `claude-opus-5-thinking-high`)
