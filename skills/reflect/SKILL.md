...
One message, three `Agent` calls, `subagent_type: general-purpose`, explicit `model:` on each. Reviewers need MCP access for context lookups (tickets, chat threads, observability traces referenced in the transcript); `general-purpose` has full tool access by default.
...
One `Agent` call, `subagent_type: general-purpose`, using your configured reflect-judgment model (default `claude-opus-5-thinking-high`). The synthesizer's quality check includes spot-verifying citations, which can require MCP access; `general-purpose` has full tool access by default. Use `references/synthesizer.md` verbatim, with each reviewer's full output inlined where marked. The synthesizer returns a structured Accepted / Rejected / Backlog list.
